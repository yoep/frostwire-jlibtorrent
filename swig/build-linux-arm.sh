...
export run_bjam="./b2 -j8 --user-config=${CURRENT_DIR}/config/${os_build}-${os_arch}-config.jam variant=release toolset=gcc-${os_arch} target-os=${os_build} location=${CURRENT_DIR}/bin/release/${os_build}/${os_arch}"
